...
public class QuarantineSystem {
    public static class DashBoard {
        ...
        public void runDashBoard() {
            ...
            for(Person p:People) {
            	if(p.getInfectCnt()>0) {
            		patientNums.set(getGroup(p.getAge()),patientNums.get(getGroup(p.getAge()))+1);
            		infectNums.set(getGroup(p.getAge()),infectNums.get(getGroup(p.getAge()))+p.getInfectCnt());         	
               	}
            	if(p.getIsVac())
            		vacNums.set(getGroup(p.getAge()),vacNums.get(getGroup(p.getAge()))+1);
            	if(p.getInfectCnt()>0&&p.getIsVac()) 
            		vacInfectNums.set(getGroup(p.getAge()),vacInfectNums.get(getGroup(p.getAge()))+1);
            }
        }
        
        private int getGroup(int age) {
        	if(age<10) 
        		return 0;
        	if(age>=10&&age<20)
        		return 1;
    ...
    public void saveSinglePatient(Record record) {
    	...
    	if(record.getStatus()==Status.Confirmed) {
    		...
    		for(Person p:People) {
    			if(p.getIDCardNo().equals(record.getIDCardNo())) {
    				person = p;
    				break;
    			}
    		}
    		...
    		for(Hospital h:Hospitals) {
    			if((hospital==null || (patient.getLoc().getDisSquare(hospital.getLoc()))>patient.getLoc().getDisSquare(h.getLoc())) && h.getCapacity().getSingleCapacity(record.getSymptomLevel())>0) {
    				hospital = h;
    			}
    		}
    		if(hospital!=null) {
				...
				newHospitalNum++;
				hospital = new Hospital("H-New-"+newHospitalNum,patient.getLoc());
			}
