...
public class NoteBookWindow extends Application {
	...
	private HBox addHBox() {
		...
		buttonLoad.setOnAction(new EventHandler<ActionEvent>() {
			public void handle(ActionEvent event) {
				...
				FileChooser.ExtensionFilter extFilter = new FileChooser.ExtensionFilter("Serialized Ovject File (*.ser)", "*.ser");
		...
		textAreaSearch.setMaxSize(400, 10);
		textAreaSearch.setPrefWidth(400);
		textAreaSearch.setPrefHeight(10);
		
		Button buttonSearch = new Button("Search");
		buttonSearch.setPrefHeight(20);
		buttonSearch.setOnAction(new EventHandler<ActionEvent>() {
			public void handle(ActionEvent event) {
				currentSearch = textAreaSearch.getText();
		...
		hbox.getChildren().addAll(new Label("Search : "),textAreaSearch, buttonSearch,buttonRemove);
	...
	private VBox addVBox() {
				...
				for(Folder f:noteBook.getFolders()) {
					if(f.getName()==currentFolder) {
						for(Note n:f.getNotes()) {
							if(n.getTitle()==title) {
								textAreaNote.setText(((TextNote)n).getContent());
								currentNote=title;
								return;
							}
						}
		...
		buttonAddNote.setOnAction(new EventHandler<ActionEvent>() {
			public void handle(ActionEvent event) {
				if(currentFolder=="-----"||currentFolder==""||currentFolder==null) {
					Alert alert = new Alert(AlertType.INFORMATION);
					alert.setTitle("Warning");
	...
	private void updateListView(boolean  search) {
		...
		if(search) {
			for(Note n:noteBook.searchNotes(currentSearch)) {
				if(list.contains(n.getTitle())) {
					temp.add(n.getTitle());
				}
			}
			list = temp;
